...
import { ArrowDownCircle, ArrowUpCircle, ArrowUpRight, CalendarDays, ChevronDown, ChevronLeft, ChevronRight, Download, Edit3, MoreHorizontal, Moon, PiggyBank, Plus, RefreshCw, Settings2, Sun, Tag, Target, Trash2, User, X } from 'lucide-react';
import { supabase } from '@/lib/supabase';

type Category = { id: string; name: string; color: string };
type SavingsGoal = { id: string; name: string; target_amount: number; color: string };
type EntryType = 'expense' | 'income' | 'savings';
type Expense = { id: string; amount: number; category_id: string | null; expense_date: string; entry_type: EntryType; is_recurring: boolean; savings_goal_id: string | null };
type Period = 'month' | 'year';

const formatMoney = (amount: number) => new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 2 }).format(amount);
const formatDate = (date: string) => new Intl.DateTimeFormat('fr-FR', { day: '2-digit', month: 'short' }).format(new Date(`${date}T12:00:00`));
const formatDateFull = (date: string) => new Intl.DateTimeFormat('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(`${date}T12:00:00`));
const today = () => new Date().toISOString().slice(0, 10);
const MONTHS = ['Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Décembre'];
const DEFAULT_FIRST_NAME = 'Marie';
const PALETTE = ['#F59E0B', '#3B82F6', '#10B981', '#F97316', '#EF4444', '#8B5CF6', '#EC4899', '#14B8A6', '#6366F1', '#84CC16'];
...
function App() {
  ...
  const [profileInput, setProfileInput] = useState(DEFAULT_FIRST_NAME);
  ...
  const loadData = async () => {
    setIsLoading(true);
    const [{ data: categoryData, error: categoryError }, { data: expenseData, error: expenseError }, { data: goalData, error: goalError }] = await Promise.all([
      supabase.from('budget_categories').select('id, name, color').order('created_at'),
      supabase.from('budget_expenses').select('id, amount, category_id, expense_date, entry_type, is_recurring, savings_goal_id').order('expense_date', { ascending: false }).limit(300),
      supabase.from('savings_goals').select('id, name, target_amount, color').order('created_at'),
    ]);
    if (categoryError || expenseError || goalError) {
  ...
  const recentEntries = [...visibleExpenses].sort((a, b) => b.expense_date.localeCompare(a.expense_date)).slice(0, 6);
    ...
    if (evolutionRange === '7days') {
      for (let i = 6; i >= 0; i--) {
        ...
        buckets.push({
          ...
          expense: dayEntries.filter((e) => e.entry_type === 'expense').reduce((s, e) => s + Number(e.amount), 0),
          income: dayEntries.filter((e) => e.entry_type === 'income').reduce((s, e) => s + Number(e.amount), 0),
        });
      }
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        buckets.push({
          ...
          expense: monthEntries.filter((e) => e.entry_type === 'expense').reduce((s, e) => s + Number(e.amount), 0),
          income: monthEntries.filter((e) => e.entry_type === 'income').reduce((s, e) => s + Number(e.amount), 0),
        });
      }
    } else {
      for (let i = 11; i >= 0; i--) {
  ...
  const savingsProjectionMonths = isRecurring ? [1, 3, 6, 12, 24] : [];
  ...
  const addExpense = async (event: FormEvent) => {
    ...
    const { data, error } = await supabase.from('budget_expenses').insert({ amount: numericAmount, category_id: categoryId || null, expense_date: expenseDate, entry_type: entryType, is_recurring: isRecurring, savings_goal_id: entryType === 'savings' ? (savingsGoalId || null) : null }).select('id, amount, category_id, expense_date, entry_type, is_recurring, savings_goal_id').single();
  ...
  const saveProfile = (event: FormEvent) => {
    event.preventDefault();
    const name = profileInput.trim() || DEFAULT_FIRST_NAME;
    setFirstName(name);
  ...
  const exportCsv = () => {
    const rows = [['Date', 'Type', 'Montant', 'Catégorie', 'Récurrent']];
    [...visibleExpenses].sort((a, b) => a.expense_date.localeCompare(b.expense_date)).forEach((e) => {
      ...
      rows.push([e.expense_date, e.entry_type === 'income' ? 'Revenu' : e.entry_type === 'savings' ? 'Épargne' : 'Dépense', String(e.amount).replace('.', ','), cat, e.is_recurring ? 'Oui' : 'Non']);
    });
  ...
  // Génération des jours pour le calendrier sur-mesure
  ...
  return (
    <main className="app-shell">
      ...
      <section className="hero-row">
        <div>
          <p className="eyebrow">THE 1% — Votre espace budget</p>
          <h1>Bonjour, {firstName}<span className="accent">.</span></h1>
          <p className="subtitle">Gérez vos dépenses, revenus et épargne, sans prise de tête.</p>
        </div>
          ...
          <section className="overview-grid">
            <div className="card chart-card">
              <div className="card-heading">
                <div>
                  ...
                  <div className="total">{formatMoney(totalSpent)}</div>
                </div>
              ...
              <div className="chart-area">
                <div className="donut" style={{ background: `conic-gradient(${donut})` }}>
                  <div className="donut-hole"><span>Total dépensé</span><strong>{formatMoney(totalSpent)}</strong></div>
                </div>
            ...
            <div className="side-stack">
              <div className="card balance-card">
                ...
                <div className={`balance-amount ${balance >= 0 ? 'positive' : 'negative'}`}>{balance >= 0 ? '+' : ''}{formatMoney(balance)}</div>
                <div className="balance-detail">
                  <div>Revenus<strong style={{ color: 'var(--good)' }}>{formatMoney(totalIncome)}</strong></div>
                  <div>Dépenses<strong style={{ color: 'var(--bad)' }}>{formatMoney(totalSpent)}</strong></div>
                  <div>Épargne<strong style={{ color: 'var(--accent)' }}>{formatMoney(totalSavings)}</strong></div>
                </div>
              </div>
              {monthlyBudget > 0 ? (
                <div className="card budget-card">
                  ...
                  <div className="budget-stats">
                    <span>{formatMoney(totalSpent)} / {formatMoney(monthlyBudget)}</span>
                    <span>{budgetRemaining >= 0 ? `${formatMoney(budgetRemaining)} restant` : `${formatMoney(-budgetRemaining)} dépassé`}</span>
                  </div>
                </div>
              ) : (
                <div className="card budget-card">
      ...
      {period === 'year' && (
        <section className="card year-card">
          <div className="card-heading">
            <div>
              <p className="label">Dépenses de {periodLabel}</p>
              <div className="total">{formatMoney(yearExpenseTotal)}</div>
            </div>
            ...
            {monthTotals.map((month) => {
              ...
              return (
                <button key={month.index} className={`month-tile ${month.total > 0 ? 'has-data' : ''} ${isActive ? 'active' : ''} ${isCurrent ? 'current' : ''}`} onClick={() => selectMonth(month.index)}>
                  ...
                  <strong>{month.total > 0 ? formatMoney(month.total) : '—'}</strong>
                </button>
              );
            })}
          </div>
      ...
      <section className="content-grid">
        <div className="card add-card" style={{ zIndex: 30, position: 'relative' }}>
          ...
          <form onSubmit={addExpense}>
                  ...
                  <div className="custom-datepicker-popover">
                    <div className="datepicker-header">
                      <button 
                        ...
                        onClick={() => setPickerViewDate(new Date(pickerViewDate.getFullYear(), pickerViewDate.getMonth() - 1, 1))}
                      >
                        <ChevronLeft size={16} />
                      </button>
                    ...
                    <div className="datepicker-days">
                      {calendarDays.map((d, idx) => (
                        <button
                          key={idx}
                          type="button"
                          className={`datepicker-day ${!d.isCurrentMonth ? 'muted' : ''} ${d.dateStr === expenseDate ? 'selected' : ''}`}
                          onClick={() => {
                            setExpenseDate(d.dateStr);
            ...
            {entryType === 'savings' && savingsProjectionAmount > 0 && savingsProjectionMonths.length > 0 && (
              <div className="savings-projection">
                <div className="projection-title"><RefreshCw size={14} /> Projection de ton épargne</div>
                <div className="projection-grid">
                  {savingsProjectionMonths.slice(1).map((m) => (
                    <div key={m} className={`projection-item ${m === 12 ? 'highlight' : ''}`}>
                      <span>{m === 3 ? '3 mois' : m === 6 ? '6 mois' : m === 12 ? '1 an' : `${m} mois`}</span>
                      <strong>{formatMoney(savingsProjectionAmount * m)}</strong>
                    </div>
                  ))}
                </div>
                <p className="projection-hint">Basé sur {formatMoney(savingsProjectionAmount)}/mois récurrent — objectifs indicatifs.</p>
              </div>
            )}
            {entryType === 'savings' && savingsProjectionAmount > 0 && !isRecurring && (
              <div className="savings-projection">
                ...
                <div className="projection-grid">
                  <div className="projection-item highlight"><span>Aujourd’hui</span><strong>{formatMoney(savingsProjectionAmount)}</strong></div>
              ...
              return (
                <div className="expense-row" key={entry.id}>
                  ...
                  <div className="expense-info">
                    ...
                    <span>{formatDate(entry.expense_date)}{entry.is_recurring && <span className="recurring-pill"><RefreshCw size={9} /> Récurrent</span>}</span>
                  </div>
                  <strong className={`expense-amount ${isIncome ? 'income' : ''} ${isSavings ? 'savings' : ''}`}>{isIncome ? '+' : isSavings ? '★' : '−'} {formatMoney(Number(entry.amount))}</strong>
              ...
              <div className="goal-card" key={goal.id}>
                <div className="goal-card-head">
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div className="goal-sub">{goal.target > 0 ? `Objectif : ${formatMoney(goal.target)}` : 'Sans objectif défini'}</div>
                  </div>
                ...
                {goal.target > 0 && (
                  <>
                    ...
                    <div className="goal-stats">
                      <span>{formatMoney(goal.saved)} épargné</span>
                      <span><strong>{Math.round(goal.ratio * 100)}%</strong></span>
                    </div>
                    <div className="goal-stats">
                      <span>Reste <strong>{formatMoney(goal.remaining)}</strong> à économiser</span>
                    </div>
                  </>
                )}
                {goal.target === 0 && (
                  <div className="goal-stats"><span><strong>{formatMoney(goal.saved)}</strong> épargné au total</span></div>
                )}
              </div>
            ))}
          </div>
